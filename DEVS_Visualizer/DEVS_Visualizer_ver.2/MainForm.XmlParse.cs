...
namespace DEVS_Visualizer_ver._2
{
    partial class MainForm
    {
            ...
            try
            {
                ...
                foreach (XElement node in root.Elements())
                {
                    if (node.Name == "OutmostModel")
                    {
                        ...
                        OutmostModel = new Model(node.Attribute("name").Value);
        ...
        private void ParsingXML(IEnumerable<XElement> elements, int depth)
        {
            foreach (var ele in elements)
            {
                if (ele.Name.ToString() == "Model")
                {
                    Model tModule = new Model(ele.Attribute("name").Value, "S");
                    ...
                    foreach (var e in ele.Elements("SubModel"))
                    {
                        Model temp = new Model(e.Attribute("name").Value, "SI");
        ...
        private void parsingXmlData(IEnumerable<XElement> nodes, ref Model temp, int depth)
        {
                ...
                switch (node.Name.ToString())
                {
                    ...
                    case "Modules":
                        foreach (XElement subNode in node.Elements())
                        {
                            ...
                            Model subModule = new Model(subNode.Attribute("name").Value);
