...
namespace DEVS_Visualizer
{
    public partial class MainForm : Form
    {
        ...
        public MainForm()
        {
            ...
            OutmostModel = new Module("TopModule");
            ...
            ReadXMLData();
            Form_Update();
            OutmostModelName = OutmostModel.Name;
        }
        ...
        {
            // Structure list
            TreeNode topNode = new TreeNode(OutmostModel.Name);

            for (int i = 0; i < OutmostModel.Instances.Count; i++)
            {
                TreeNode temp;
                if (OutmostModel.Instances[i].Instances.Count > 0)
                {
                    temp = new TreeNode(OutmostModel.Instances[i].Name);
                    TreeList_AddInstance(ref temp, OutmostModel.Instances[i].Instances);
                    topNode.Nodes.Add(temp);
                }
                else if (OutmostModel.Instances[i].Type != "None")
                {
                    temp = new TreeNode(OutmostModel.Instances[i].Name);
                    TreeList_AddInstance(ref temp, ModulePool[OutmostModel.Instances[i].Type].Instances);
                    topNode.Nodes.Add(temp);
                }
                else
                {
                    topNode.Nodes.Add(OutmostModel.Instances[i].Id, OutmostModel.Instances[i].Name);
                }
            }
        ...
        {
            if (OutmostModel.Name == instanceName)
                return OutmostModel;

            for (int i = 0; i < OutmostModel.Instances.Count; i++)
            {
                if (OutmostModel.Instances[i].Name == instanceName)
                    return OutmostModel.Instances[i];
            }

            for (int i = 0; i < OutmostModel.Instances.Count; i++)
            {
                if (OutmostModel.Instances[i].Instances.Count > 0)
                    return find_instance_recur(OutmostModel.Instances[i].Instances, instanceName);
            }
        ...
        private void treeView1_MouseDoubleClick(object sender, MouseEventArgs e)
        {
            ...
            nDrawingView.LockRefresh = true;
            nDrawingDocument.ActiveLayer.RemoveAllChildren();

            Update_instance(finded_instance_update(find_instance(node.Text)));

            nDrawingView.LockRefresh = false;
